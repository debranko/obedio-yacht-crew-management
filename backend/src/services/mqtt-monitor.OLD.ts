 ...
 * A separate web interface for monitoring MQTT connections and messages
...
<html>
<head>
  <title>OBEDIO MQTT Monitor</title>
  <style>
...
<body>
  ...
  <div class="controls">
    ...
    <div class="filter">
      <label>Filter:</label>
      <input type="text" id="filter" placeholder="Filter by topic or device..." onkeyup="filterMessages()">
    </div>
  ...
  <script>
    ...
    function updateMessages() {
      ...
      if (filtered.length === 0) {
        container.innerHTML = '<div class="empty">No messages match filter</div>';
        return;
      }
  ...
  private setupSocketIO() {
    this.io.on('connection', (socket) => {
      console.log('📟 MQTT Monitor client connected');
  ...
  public start() {
    this.httpServer.listen(this.port, () => {
      console.log(`
🖥️  MQTT Monitor Dashboard Started!

📍 Access URL: http://localhost:${this.port}

This monitoring dashboard shows:
• All MQTT messages in real-time
• Connected devices and their status
• Message filtering and search
• Export logs for debugging

Keep this window open to monitor ESP32 button activity!
      `);
    });
  }
}
