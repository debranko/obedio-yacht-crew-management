...
export class DatabaseService {
  ...
  async cancelServiceRequest(requestId: string) {
    // First, get the current request to check its state
    ...
    // Prevent cancelling already completed or cancelled requests
    ...
    const request = await this.prisma.serviceRequest.update({
      ...
      data: {
        status: 'cancelled',
        completedAt: new Date()  // Reuse completedAt for cancelled timestamp
      },
      include: {
        guest: true,
        location: true,
    ...
    // Create history record
    const responseTime = request.acceptedAt
      ? Math.floor((request.acceptedAt.getTime() - request.createdAt.getTime()) / 1000)
      : null;
    ...
    await this.prisma.serviceRequestHistory.create({
      data: {
        ...
        action: 'cancelled',
        newStatus: 'cancelled',
        completedBy: request.assignedTo || null,
        completedAt: request.completedAt!,
        responseTime,
        completionTime: null,  // No completion time for cancelled requests
        guestName: request.guestName,
        location: request.location?.name || request.guestCabin || null,
        requestType: request.requestType,
        priority: request.priority
    ...
    const guestName = request.guest
      ? `${request.guest.firstName} ${request.guest.lastName}`
      : request.guestName || 'Guest';
    const locationName = request.location?.name || request.guestCabin || 'Unknown';

    await this.prisma.activityLog.create({
      data: {
        type: 'service_request',
        action: 'Request Cancelled',
        details: `Service request cancelled for ${guestName} at ${locationName}${request.assignedTo ? ` (was assigned to ${request.assignedTo})` : ''}`,
        userId: request.crewmember?.userId || null,
        locationId: request.locationId,
        guestId: request.guestId,
        metadata: JSON.stringify({
          ...
          requestType: request.requestType,
          priority: request.priority,
          wasAssigned: !!request.assignedTo,
